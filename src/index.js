...
module.exports = app => {
  app.on(['pull_request.opened', 'pull_request.reopened', 'pull_request.edited'], async context => {
    ...
    const allFiles = await context.github.paginate(context.github.pullRequests.listFiles(issue), res => res.data);
    ...
    if (flatLines.length > config.minChangedLines && flatLines.filter(e => e.startsWith('+//')).length === 0) {
      const issueComment = context.issue({
        body: 'You changed a lot of lines of code but did not add any comments. Shame on you! Shame!!!'
      });
    ...
    if (config.docscheck) {
      ...
      if (docsNeedToBeChecked.length > 0 && allFiles.filter(e => e.filename.startsWith('docs')).length === 0) {
        const issueComment = context.issue({
          body: "You added and/or removed files but didn't make any docs changes. What the hell's wrong with you?!?!"
        });
        context.github.issues.createComment(issueComment);
      }
    }
